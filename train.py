...
def train(train_iter, valid_iter, src_vocab, tgt_vocab, epochs=EPOCHS):
    ...
    for epoch in range(1, epochs + 1):
        ...
        for src, tgt in train_iter:
            ...
            p_bar.set_postfix(epoch=f" {epoch}, train loss= {round(sum(train_loss) / len(train_loss), 4)}, valid loss = +inf",
                             refresh=True)
            p_bar.update()
        ...
        p_bar.set_postfix(
            epoch=f" {epoch}, train loss= {round(sum(train_loss) / len(train_loss), 4)}, valid loss = {round(sum(train_loss) / len(train_loss), 4)}",
            refresh=False)
        p_bar.close()

        # Save the checkpoint
        # save_model(seq2seq, src_vocab, tgt_vocab, f'checkpoints/CHECKPOINT_WITHOUTO_ATT__EN__TO__DE__EPOCH_{epoch}__AT__{strftime("%Y_%m_%d__%H_%M_%S", gmtime())}__TRAIN_LOSS__{round(sum(training_loss) / len(training_loss))}')
